...
/** Build a self-consistent history the way a real hand would produce one. */
function buildHistory(): HandHistory & { seats: HistorySeat[]; board: number[] } {
  ...
  return {
    ...
    board: Array.from(deck.slice(4, 9)),
    seats: [
      {
        index: 0,
        dealtIn: true,
...
describe("verify", () => {
  ...
  it("catches a board that does not come from the seed", () => {
    ...
    const result = verify(h);
    expect(result.ok).toBe(false);
    expect(result.problems.join(" ")).toContain("but the deck gives");
  });

  it("catches hole cards that were not dealt", () => {
    const h = buildHistory();
    h.seats[1].revealed = [51, 50];
    const result = verify(h);
    expect(result.ok).toBe(false);
    expect(result.problems.join(" ")).toContain("seat 1");
  });
